...
// 当前智能体名称（营销页固定显示 HuizhouPowerQAAgent 对应名称）
const currentAgentName = computed(() => {
  if (!agents.value || !agents.value.length) return 'HuizhouPowerQAAgent'
  const agent = agents.value.find((a) => a.id === MARKETING_AGENT_ID)
  return agent ? agent.name : 'HuizhouPowerQAAgent'
})
...
// 获取对话线程列表（营销页固定 agent + agent_config_id）
const fetchThreads = async () => {
  isLoadingThreads.value = true
  try {
    const fetchedThreads = await threadApi.getThreads(MARKETING_AGENT_ID, {
...
// 获取线程消息（营销页固定 agent + agent_config_id）
const fetchThreadMessages = async (threadId, delay = 0) => {
  ...
  try {
    const response = await agentApi.getAgentHistory(MARKETING_AGENT_ID, threadId, {
      agent_config_id: MARKETING_AGENT_CONFIG_ID
    })
...
const selectChat = async (chat) => {
  ...
  try {
    await fetchThreadMessages(chat.id)
  } catch (error) {
...
// 营销页固定智能体，不随 store 切换而清空；仅首次进入或需刷新时由 onMounted 拉取
...
// 确保有活动的线程（营销页固定 agent + agent_config_id）
const ensureActiveThread = async (preferredTitle = '新的对话') => {
  ...
  try {
    const thread = await threadApi.createThread(MARKETING_AGENT_ID, preferredTitle, {
      agent_config_id: MARKETING_AGENT_CONFIG_ID
    })
    if (thread) {
...
// 显式创建一个全新的对话（点击“新建对话”按钮时使用，营销页固定 agent + agent_config_id）
const createNewChat = async () => {
  try {
    const thread = await threadApi.createThread(MARKETING_AGENT_ID, '新的对话', {
      agent_config_id: MARKETING_AGENT_CONFIG_ID
    })
    if (thread) {
...
// 发送消息（营销页固定使用 HuizhouPowerQAAgent，config 含 agent_config_id）
const sendMessage = async ({ threadId, text, imageData, attachments, signal }) => {
  // 如果是新对话，用消息内容作为标题
  ...
  try {
    return await agentApi.sendAgentMessage(MARKETING_AGENT_ID, requestData, signal ? { signal } : undefined)
  } catch (error) {
...
const handleSendOrStop = async () => {
  ...
  if (isProcessing.value && threadState && threadState.streamAbortController) {
    ...
    // 中断后刷新消息历史，确保显示最新的状态
    try {
      await fetchThreadMessages(threadId, 500)
...
const handleSend = async () => {
  ...
  if (!text && !currentImage.value && currentAttachments.value.length === 0) {
    return
  }
  ...
  try {
    ...
    fetchThreadMessages(threadId, 500).finally(() => {
...
const { handleAgentResponse } = useAgentStreamHandler({
  ...
  currentAgentId: marketingAgentIdRef,
